...
def process_params(param, kind):
    ...
    layouts['width'] = param.pop('fig_width', None)
    layouts['height'] = param.pop('fig_height', None)
    ...
    # Specific parameters
    #if kind == 'imshow':
    #    param['labels'] = dict(color=param.pop('colorbar_title', None))
...
accepted_params = {'title': 'Titolo del grafico'}
...
def mmp_plot(data=None, kind='lines', x=None, y=None, z=None, show=True, mmp_logo=False, proj_logo_source=None, **param):
    ...
    #    data = data.set_index([xp, yp]).unstack()
